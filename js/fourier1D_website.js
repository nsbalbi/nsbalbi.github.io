...
function Vector(radius, speed, color) {
    this.x = canvas.width/5; // Initial center of circle
...
function preset_1() {
    ...
    for (var i = 2; i <= numCircles; i += 2) {
        circles.push(new Vector(circles[0].radius/i,  i * 0.01, randColor()));
        circles.push(new Vector(circles[0].radius/(i+1), -(i+1) * 0.01, randColor()));
    }
    if (numCircles % 2 == 0) {
        circles.pop();
    }
...
function animate() {
    ...
    // Updates each circle position in order
    ...
    // Logs the current value, and deletes old ones
    plot.unshift(circles[circles.length - 1].nexty);
    if (plot.length > 800) {
        plot.pop();
    }

    // Plots recorded values over time
    c.beginPath();
    c.strokeStyle = "white";
    c.moveTo(circles[circles.length - 1].nextx, circles[circles.length - 1].nexty);
    c.lineTo(600, circles[circles.length - 1].nexty);
    for (var i = 0; i < plot.length; i++) {
        c.lineTo(i + 600, plot[i]);
    }
    c.stroke();
}
